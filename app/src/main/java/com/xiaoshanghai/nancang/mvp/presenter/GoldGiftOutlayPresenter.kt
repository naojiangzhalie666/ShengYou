...
import com.xiaoshanghai.nancang.mvp.model.GoldGiftOutlayModel
...
class GoldGiftOutlayPresenter : BasePresenter<GoldGiftOutlayContract.View>(), GoldGiftOutlayContract.Presenter {
    ...
    private var model: GoldGiftOutlayModel? = GoldGiftOutlayModel()

    override fun selectTime(date: String?) {
        val calendar = Calendar.getInstance()
        calendar.time = DateUtil.stringToDate(date, DateUtil.DatePattern.ONLY_DAY)
        val pvTime = TimePickerBuilder((view as GoldGiftOutlayFragment).context, OnTimeSelectListener { date, v -> //选中事件回调
            ...
            view.setTime(dateToString)
                ...
                .setDecorView((view as GoldGiftOutlayFragment).activity!!.window.decorView.findViewById(R.id.content))
                .build()
        pvTime.show()
    }


    override fun getGoldGiftOutlayList(refresh: RefreshLayout?, date: String?) {
        val replace = date?.replace("-", "/")

        model!!.getGoldGiftOutlayList(mPage.toString() + "", size.toString() + "", replace)
