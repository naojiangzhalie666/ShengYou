...
class RoomTalkBarrage : LinearLayout {
    ...
    lateinit var mIvClv: UserLevelView

    @BindView(R.id.tv_name)
    lateinit var mTvName: TextView

    @BindView(R.id.tv_takl)
    lateinit var mTvTakl: TextView
    ...
    private fun init(context: Context?) {
        mContext = context!!
        mView = LayoutInflater.from(context).inflate(R.layout.view_room_talk_barrage, this, true)
...
//        }
        ...
        mIvClv.setUserLevel(msgBody?.userInfo?.userLevel!!)

        var myUserInfo = SPUtils.getInstance().userInfo

        if (myUserInfo.id == msgBody.userInfo.id) {
            mTvName.text = "我"
        } else {
            mTvName.text = msgBody.userInfo?.userName
        }
        mTvTakl.text = msgBody.message
