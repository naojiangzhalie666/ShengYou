...
import com.xiaoshanghai.nancang.mvp.model.MyFamilyIncomeModel
...
class MyFamilyIncomePresenter : BasePresenter<MyFamilyIncomeContract.View>(), MyFamilyIncomeContract.Presenter {

    var initPage = 1
    var mPage = initPage
    var size = 10

    private val model by lazy { MyFamilyIncomeModel() }
    ...
    override fun getFamilyIncome(refresh: RefreshLayout?, userId: String?, clanId:String?,startTime: String?) {
        val replace: String = startTime!!.replace("-", "/")
        model.getFamilyIncome(mPage.toString(), size.toString(), userId,clanId, replace)
