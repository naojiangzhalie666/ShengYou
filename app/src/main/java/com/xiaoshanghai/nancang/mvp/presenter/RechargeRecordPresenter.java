...
import com.xiaoshanghai.nancang.mvp.model.RechargeRecordModel;
import com.xiaoshanghai.nancang.mvp.ui.activity.mine.RechargeRecordAct;
...
public class RechargeRecordPresenter extends BasePresenter<RechargeRecordContract.View> implements RechargeRecordContract.Presenter {
    ...
    public void selectTime(String date) {
        ...
        TimePickerView pvTime = new TimePickerBuilder(((RechargeRecordAct)getView()), new OnTimeSelectListener() {
            @Override
...
//                .setContentSize(18)//滚轮文字大小
                ...
                .setDecorView(((RechargeRecordAct) (getView())).getWindow().getDecorView().findViewById(android.R.id.content))
                .build();
        pvTime.show();
    }

    @Override
    public void getIncomeList(RefreshLayout refresh,String userId, String date) {
        String replace = date.replace("-", "/");
        model.getGoldRecharge(mPage + "", size + "",userId,replace)
                .execOnThread(getView().getActLifeRecycle(), new HttpObserver<HomeRoomResult<List<CashEntity>>>() {
                    @Override
                    protected void success(HomeRoomResult<List<CashEntity>> bean, BaseResponse<HomeRoomResult<List<CashEntity>>> response) {
                        List<CashEntity> records = bean.getRecords();
//                        if (records != null) {
                        if (mPage == initPage) {
