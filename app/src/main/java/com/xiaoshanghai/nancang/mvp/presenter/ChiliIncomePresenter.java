...
import com.xiaoshanghai.nancang.mvp.model.ChiliIncomeModel;
...
public class ChiliIncomePresenter extends BasePresenter<ChiliIncomeContract.View> implements ChiliIncomeContract.Presenter {

    public int initPage = 1;
    public int mPage = initPage;
    public int size = 10;

    private ChiliIncomeModel model;

    public ChiliIncomePresenter() {
        model = new ChiliIncomeModel();
    }
    ...
    public void selectTime(String date) {
        ...
        TimePickerView pvTime = new TimePickerBuilder(((ChiliIncomeFragment)getView()).getContext(), new OnTimeSelectListener() {
            @Override
...
//                .setContentSize(18)//滚轮文字大小
                ...
                .setDecorView(((ChiliIncomeFragment) (getView())).getActivity().getWindow().getDecorView().findViewById(android.R.id.content))
                .build();
        pvTime.show();
    }
    ...
    public void getIncomeList(RefreshLayout refresh, String date) {
        ...
        model.getMyChiliIncomeList(mPage + "", size + "",replace)
