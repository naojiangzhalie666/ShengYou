...
import com.xiaoshanghai.nancang.mvp.model.ChiliGiftIncomeModel
...
class ChiliGiftIncomePresenter : BasePresenter<ChiliGiftIncomeContract.View>(), ChiliGiftIncomeContract.Presenter {

    var initPage = 1
    var mPage = initPage
    var size = 10

    val model:ChiliGiftIncomeModel by lazy { ChiliGiftIncomeModel() }
    ...
    override fun selectTime(date: String?) {
        ...
        val pvTime = TimePickerBuilder((view as ChiliGiftIncomeFragment).context, OnTimeSelectListener { date, v -> //选中事件回调
            ...
            view.setTime(dateToString)
                ...
                .setDecorView((view as ChiliGiftIncomeFragment).activity!!.window.decorView.findViewById(R.id.content))
                .build()
        pvTime.show()
    }

    override fun getGoldGiftList(refresh: RefreshLayout?, date: String?) {
        val replace = date?.replace("-", "/")
        model!!.getChiliGiftList(mPage.toString() + "", size.toString() + "", replace)
