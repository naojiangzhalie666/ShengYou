...
import com.xiaoshanghai.nancang.mvp.presenter.GoldGiftOutlayPresenter
...
class GoldGiftOutlayFragment : BaseMvpFragment<GoldGiftOutlayPresenter>(), GoldGiftOutlayContract.View, OnRefreshLoadMoreListener {
    ...
    companion object {
        fun newInstance(): GoldGiftOutlayFragment {
            return GoldGiftOutlayFragment()
        }
    }

    override fun createPresenter(): GoldGiftOutlayPresenter = GoldGiftOutlayPresenter()

    override fun setLayoutId(): Int = R.layout.fragment_gold_gift_outlay
    ...
    private fun init() {
        ...
        mAdapter = GoldGiftIncomeAdapter(header, Constant.MY_GIFT_GOLD_OUTLAY)
    ...
    override fun initView(savedInstanceState: Bundle?) {
        ...
        mPresenter.getGoldGiftOutlayList(null, tvDate.text.toString().trim ())
    }
    ...
    fun onViewClicked(view: View) {
        when (view.id) {
            R.id.iv_calendar -> mPresenter.selectTime(tvDate.getText().toString().trim ())
            R.id.iv_now_day -> {
                val date = Date()
                val dateToString = DateUtil.dateToString(date, DateUtil.DatePattern.ONLY_DAY)
                tvDate.setText(dateToString)
                mPresenter.mPage = mPresenter.initPage
                mPresenter.getGoldGiftOutlayList(null, tvDate.getText().toString().trim())
            }
        }
    }
    ...
    override fun onRefresh(refreshLayout: RefreshLayout) {
        mPresenter.mPage = mPresenter.initPage
        mPresenter.getGoldGiftOutlayList(refreshLayout, tvDate.getText().toString().trim())
    }
