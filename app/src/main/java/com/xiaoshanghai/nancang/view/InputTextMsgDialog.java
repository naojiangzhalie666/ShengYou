...
public class InputTextMsgDialog extends Dialog {
    ...
    private String atFlat = "";

    public InputTextMsgDialog(Context context, int theme) {
        super(context, theme);
        mContext = context;
        setContentView(R.layout.dialog_message_details_input);
    ...
    public void setAtFlat(String atFlat) {
        this.atFlat = atFlat;
        if (!TextUtils.isEmpty(atFlat)) {
            mEditMessage.setHint("回复 " + atFlat);
        } else {
            mEditMessage.setHint("");
        }
    }
