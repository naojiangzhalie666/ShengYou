...
public class HomeGridDecoration extends RecyclerView.ItemDecoration {
    ...
    public HomeGridDecoration(int horizontal, int vertical, int leftMargin, int rightMargin, int spanCount) {
...
//        if (Math.floor(position / mSpanCount) == 0) {       //第一行

            if ((position + 1) % mSpanCount == 1) {            //第一列
            ...
            } else if ((position + 1) % mSpanCount == 0) {       //最后一列
