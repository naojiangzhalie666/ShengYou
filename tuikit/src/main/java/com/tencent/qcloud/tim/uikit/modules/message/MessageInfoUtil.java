...
public class MessageInfoUtil {
    ...
    public static MessageInfo buildTextAtMessage(List<String> atUserList, String message) {
        ...
        for (String userId : atUserList) {
            MessageInfo atMessage = new MessageInfo();
            V2TIMMessage testMessage = V2TIMManager.getMessageManager().createTextMessage("我在交友大厅发现好玩的东西！ @你了 快点过来看看");

            atMessage.setExtra("我在交友大厅发现好玩的东西！ @你了 快点过来看看");
            atMessage.setMsgTime(System.currentTimeMillis() / 1000);
            atMessage.setSelf(true);
            atMessage.setTimMessage(testMessage);
            atMessage.setFromUser(V2TIMManager.getInstance().getLoginUser());
            atMessage.setMsgType(MessageInfo.MSG_TYPE_TEXT);

            V2TIMMessage messageTim = atMessage.getTimMessage();
            V2TIMManager.getMessageManager().sendMessage(messageTim, userId,  null,
