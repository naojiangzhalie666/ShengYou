...
//                mRoomId = String.valueOf(roomId);
                ...
                if (roomParam != null && roomParam.seatInfoList != null) {
                    for (TRTCVoiceRoomDef.SeatInfo seatInfo : roomParam.seatInfoList) {
                        ...
                        item.closeStatus = seatInfo.closeStatus;
                    ...
                    for (int i = 0; i < seatCount; i++) {
                        ...
                        txSeatInfo.status = TXSeatInfo.STATUS_UNUSED;
                        txSeatInfo.closeStatus = TXSeatInfo.STATUS_OPEN;
                        txSeatInfo.mute = false;
                        txSeatInfo.user = "";
                        txSeatInfoList.add(txSeatInfo);
                        TRTCVoiceRoomDef.SeatInfo seatInfo = new TRTCVoiceRoomDef.SeatInfo();
                        seatInfo.status = TRTCVoiceRoomDef.SeatInfo.STATUS_UNUSED;
                        seatInfo.closeStatus = TRTCVoiceRoomDef.SeatInfo.STATUS_OPEN;
                        seatInfo.mute = false;
                        seatInfo.userId = "";
                        mSeatInfoList.add(seatInfo);
                    }
...
//                        if (mUserId != null && mUserId.equals(mSeatInfoList.get(i).userId)) {
//                            mTakeSeatIndex = i;
//                            break;
//                        }
//                    }
...
//                    });
    ...
    @Override
    public void closeSeat(final boolean isClose, final TRTCVoiceRoomCallback.ActionCallback callback) {
        runOnMainThread(new Runnable() {
            @Override
            public void run() {
                TXRoomService.getInstance().closeSeat( isClose, new TXCallback() {
                    @Override
                    public void onCallback(final int code, final String msg) {
                        runOnDelegateThread(new Runnable() {
                            @Override
    ...
    @Override
    public void sendRoomTextMsg(final String message,
                                final TRTCVoiceRoomCallback.ActionCallback callback) {
        runOnMainThread(new Runnable() {
            @Override
            public void run() {
    ...
    @Override
    public void sendGlobalCustomMsg(final String cmd, final String message, final TRTCVoiceRoomCallback.ActionCallback callback) {
        runOnMainThread(new Runnable() {
            @Override
            public void run() {
                TRTCLogger.i(TAG, "sendRoomCustomMsg");
                TXRoomService.getInstance().sendGlobalCustomMsg(cmd, message, new TXCallback() {
                    @Override
                    public void onCallback(final int code, final String msg) {
                        runOnDelegateThread(new Runnable() {
                            @Override
    ...
    @Override
    public void acceptInvitation(final String id,
                                 final TRTCVoiceRoomCallback.ActionCallback callback) {
        runOnMainThread(new Runnable() {
            @Override
            public void run() {
    ...
    @Override
    public void rejectInvitation(final String id,
                                 final TRTCVoiceRoomCallback.ActionCallback callback) {
        runOnMainThread(new Runnable() {
            @Override
            public void run() {
    ...
    @Override
    public void cancelInvitation(final String id,
                                 final TRTCVoiceRoomCallback.ActionCallback callback) {
        runOnMainThread(new Runnable() {
            @Override
            public void run() {
    ...
    private void enterTRTCRoomInner(final String roomId, final String userId,
                                    final String userSig, final int role, final TRTCVoiceRoomCallback.ActionCallback callback) {
        // 进入 TRTC 房间
        ...
        runOnDelegateThread(new Runnable() {
            ...
            public void run() {
                ...
                for (TXSeatInfo seatInfo : tXSeatInfoList) {
                    ...
                    info.closeStatus = seatInfo.closeStatus;
...
//                }
    ...
    @Override
    public void onReceiveNewInvitation(final String id, final String inviter,
                                       final String cmd, final String content) {
        runOnDelegateThread(new Runnable() {
            @Override
            public void run() {
                if (mDelegate != null) {
    ...
    @Override
    public void onNetworkQuality(TRTCCloudDef.TRTCQuality
                                         trtcQuality, ArrayList<TRTCCloudDef.TRTCQuality> arrayList) {

    }

    @Override
    public void onUserVoiceVolume(final ArrayList<TRTCCloudDef.TRTCVolumeInfo> userVolumes,
                                  int totalVolume) {
        runOnDelegateThread(new Runnable() {
            @Override
            public void run() {
