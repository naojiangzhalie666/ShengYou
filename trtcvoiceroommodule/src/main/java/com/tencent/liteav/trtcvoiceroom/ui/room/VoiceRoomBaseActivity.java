...
public class VoiceRoomBaseActivity extends AppCompatActivity implements VoiceRoomSeatAdapter.OnItemClickListener, TRTCVoiceRoomDelegate, InputTextMsgDialog.OnTextSendListener, MsgListAdapter.OnItemClickListener {
    ...
    protected static final int MAX_SEAT_SIZE = 7;      //除主麦位最大麦位
    protected static final String VOICEROOM_ROOM_ID = "room_id";    //房间id
    protected static final String VOICEROOM_ROOM_NAME = "room_name";//房间名
    protected static final String VOICEROOM_USER_NAME = "user_name";//房主名
    protected static final String VOICEROOM_USER_ID = "user_id";//房主id
    protected static final String VOICEROOM_USER_SIG = "user_sig";
    protected static final String VOICEROOM_NEED_REQUEST = "need_request";  //上麦是否需要房主同意
    protected static final String VOICEROOM_SEAT_COUNT = "seat_count";
    protected static final String VOICEROOM_AUDIO_QUALITY = "audio_quality";    //音质
    protected static final String VOICEROOM_USER_AVATAR = "user_avatar";    //房主头像
    protected static final String VOICEROOM_ROOM_COVER = "room_cover";      //房间封面
    ...
    protected TRTCVoiceRoom mTRTCVoiceRoom; //直播间管理类

    protected List<VoiceRoomSeatEntity> mVoiceRoomSeatEntityList;   //座位列表
    protected VoiceRoomSeatAdapter mVoiceRoomSeatAdapter;//座位Adapter
    ...
    protected String mRoomName;//房间名
    protected String mUserName;//放主用户名
    protected String mUserAvatar;//房主头像url
    protected String mRoomCover;    //房间封面
    protected String mMainSeatUserId;//1号座id
    protected boolean mNeedRequest;// 上麦是否需要统一
    protected int mAudioQuality;//音质

    protected List<MsgEntity> mMsgEntityList;//消息列表
    protected MsgListAdapter mMsgListAdapter;//消息adapter
    protected ConfirmDialogFragment mConfirmDialogFragment;//提示框dialog
    ...
    public void onSeatListChange(final List<TRTCVoiceRoomDef.SeatInfo> seatInfoList) {
        ...
        for (int i = 0; i < seatInfoList.size(); i++) {
            ...
            // 接下来是座位区域的列表 获取麦位信息变换前的麦位信息
            VoiceRoomSeatEntity oldSeatEntity = mVoiceRoomSeatEntityList.get(i - 1);
            //新的麦位上的userid不为空并且新麦位上的userid与旧麦位上的userid不相同
...
//            }
            ...
            if (newSeatInfo.status == STATUS_UNUSED){
                oldSeatEntity.isUsed = false;//麦位上没有人
            } else if (newSeatInfo.status == STATUS_USED) {
                oldSeatEntity.isUsed = true; //麦位上有人
            }
        ...
        //所有的userId拿到手，开始去搜索详细信息了  这里应该是获取信息有变换的userId的详情
