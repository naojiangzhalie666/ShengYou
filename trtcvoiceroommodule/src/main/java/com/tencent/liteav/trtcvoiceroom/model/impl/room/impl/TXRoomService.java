...
import com.tencent.liteav.trtcvoiceroom.BuildConfig;
import com.tencent.liteav.trtcvoiceroom.constant.Constant;
...
public class TXRoomService extends V2TIMSDKListener {
    ...
    public void login(int sdkAppId, final String userId, final String userSig, final TXCallback callback) {
//        TUIKit.login(userId,userSig,null);
        // 未初始化 IM 先初始化 IM
        if (!mIsInitIMSDK) {
        ...
        V2TIMManager.getInstance().login(userId, userSig, new V2TIMCallback() {
            ...
            public void onSuccess() {
                ...
                //登录成功加入全局群
                V2TIMManager.getInstance().joinGroup(Constant.GLOBAL_SQUARD_GROUP_ID, "", new V2TIMCallback() {
                    @Override
                    public void onError(int i, String s) {
                        TRTCLogger.i(TAG, s);
                    }
...
//
                if (TUIKitConfigs.getConfigs().getGeneralConfig().isSupportAVCall()) {
                    ...
                    AVCallManager.getInstance().init(mContext);
                }
...
//        mIsInitIMSDK = true;
        ...
        V2TIMManager.getInstance().logout(new V2TIMCallback() {
            ...
            public void onSuccess() {
                ...
                //登录成功加入全局群
                V2TIMManager.getInstance().quitGroup(Constant.GLOBAL_SQUARD_GROUP_ID, null);

                if (!TUIKitConfigs.getConfigs().getGeneralConfig().isSupportAVCall()) {
                    return;
                }

                Intent intent = new Intent(mContext, AVCallManager.class);
    ...
    public void enterRoom(final String roomId, final TXCallback callback) {
        cleanStatus();
        mRoomId = roomId;
        V2TIMManager.getInstance().joinGroup(Constant.GLOBAL_GROUP_ID, "", null);
//        V2TIMManager.getInstance().joinGroup(Constant.GLOBAL_SQUARD_GROUP_ID, "", null);
    ...
    public void exitRoom(final TXCallback callback) {
        ...
        V2TIMManager.getInstance().quitGroup(Constant.GLOBAL_GROUP_ID, null);
//        V2TIMManager.getInstance().quitGroup(Constant.GLOBAL_SQUARD_GROUP_ID, null);
    ...
    public void takeSeat(int index, TXCallback callback) {
        ...
        if (info.status == TXSeatInfo.STATUS_USED) {
            ...
            if (callback != null) {
                callback.onCallback(-1, "seat is used");
            }
            return;
        }
        ...
        changeInfo.closeStatus = info.closeStatus;
...
//            return;
//        }
        int changeStatus = isClose ? TXSeatInfo.STATUS_CLOSE : TXSeatInfo.STATUS_OPEN;

        TXSeatInfo info = mTXSeatInfoList.get(index);
        if (info.closeStatus == changeStatus) {
            if (callback != null) {
        ...
        changeInfo.closeStatus = isClose ? TXSeatInfo.STATUS_CLOSE : TXSeatInfo.STATUS_OPEN;
    ...
    public void closeSeat(boolean isClose, TXCallback callback) {
//        if (!isOwner()) {
//            TRTCLogger.e(TAG, "only owner could close seat");
//            if (callback != null) {
//                callback.onCallback(-1, "only owner could close seat");
...
//        }
        ...
        for (int index = 0; index < mTXSeatInfoList.size(); index++) {
...
//                }
        ...
        modifyGroupAttrs(hashMap, callback);
    ...
    public void sendGlobalCustomMsg(String cmd, String message, final TXCallback callback) {
        if (!isEnterRoom()) {
            TRTCLogger.e(TAG, "send room custom msg fail, not enter room yet.");
            if (callback != null) {
                callback.onCallback(-1, "send room custom msg fail, not enter room yet.");
        ...
        sendGlobalGroupMsg(IMProtocol.getCusMsgJsonStr(cmd, message), callback);
    }

    public void sendGlobalGroupMsg(String data, final TXCallback callback) {
        V2TIMManager.getInstance().sendGroupCustomMessage(data.getBytes(), Constant.GLOBAL_GROUP_ID, V2TIMMessage.V2TIM_PRIORITY_NORMAL, new V2TIMValueCallback<V2TIMMessage>() {
            @Override
            public void onError(int i, String s) {
                TRTCLogger.e(TAG, "sendGroupMsg error " + i + " msg:" + s);
                if (callback != null) {
    ...
    private class VoiceRoomSimpleListener extends V2TIMSimpleMsgListener {
        ...
        public void onRecvGroupCustomMessage(String msgID, String groupID, V2TIMGroupMemberInfo sender, byte[] customData) {
            if (!groupID.equals(mRoomId) && !groupID.equals(Constant.GLOBAL_GROUP_ID)) {
                return;
            }
...
//                                break;
                    ...
                    if (oldInfo.closeStatus != newInfo.closeStatus) {
                        onSeatClose(i, newInfo.closeStatus == TXSeatInfo.STATUS_OPEN);
                    }
